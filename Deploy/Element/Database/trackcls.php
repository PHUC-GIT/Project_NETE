...
// NET.ESCAPE | (C) 2024-2026 PHUC-GIT | GNU AGPLv3 (See /LICENSE)
    ...
    class track extends database{
        ...
        public function reportin_file($reason, $fileid, $retry = 3): bool{
            // Gen Unique ID
            $uniid = $this->get_custom_id();
            // Main operation
            try {
                $add=$this->connect->prepare("INSERT INTO user_report (id, USER_ID, ITEM_TYPE, ITEM_ID, REASON) VALUE (?,?,?,?,?)");
                $add->execute(array($uniid, $_SESSION['AUTHENTICATE_USER'], 'file', $fileid, $reason));
                return $add->rowCount() > 0;
            } catch (PDOException $e) {
                if ($e->getCode() === '23000' && $retry > 0) {
                    return $this->reportin_file($reason, $fileid, $retry = 3);
                }
                return false;
            }
        }
        public function reportin_note($reason, $noteid, $retry = 3): bool{
            // Gen Unique ID
            $uniid = $this->get_custom_id();
            // Main operation
            try {
                $add=$this->connect->prepare("INSERT INTO user_report (id, USER_ID, ITEM_TYPE, ITEM_ID, REASON) VALUE (?,?,?,?,?)");
                $add->execute(array($uniid, $_SESSION['AUTHENTICATE_USER'], 'note', $noteid, $reason));
                return $add->rowCount() > 0;
            } catch (PDOException $e) {
                if ($e->getCode() === '23000' && $retry > 0) {
                    return $this->reportin_note($reason, $noteid, $retry = 3);
                }
                return false;
            }
        }
