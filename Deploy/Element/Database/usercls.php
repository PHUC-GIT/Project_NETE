...
// NET.ESCAPE | (C) 2024-2026 PHUC-GIT | GNU AGPLv3 (See /LICENSE)
    ...
    class user extends database{
        ...
        public function add_user($username, $password, $salt, $comment, $storage, $retry = 3): bool {
            // Gen Unique ID
            ...
            try {
                ...
                $add_value = $this->connect->prepare("INSERT INTO user (iduser, username, password, salt, comment, storage_allocated) VALUES (?,?,?,?,?,?)");
                $add_value->execute(array($uniid, $username, $password, $salt, $comment, $storage));

                $add_prefer = $this->connect->prepare("INSERT INTO preference (userid) VALUES (?)");
                $add_prefer->execute(array($uniid));
                ...
                if ($e->getCode() === '23000' && $retry > 0) {
                    return $this->add_user($username, $password, $salt, $comment, $storage, $retry - 1);
                }
                return false;
            }
        }
        public function user_ID($Get_NAME){
            $get_value=$this->connect->prepare("SELECT iduser, salt FROM user WHERE username COLLATE utf8mb4_bin = ?");
        ...
        public function UserCheckLogin($username, $password) {
            $select = $this->connect->prepare("SELECT password FROM user WHERE username COLLATE utf8mb4_bin = ?");
