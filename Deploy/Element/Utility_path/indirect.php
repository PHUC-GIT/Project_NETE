...
function generateAuthenticYoutubeID() {
    $chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
    // The 11th character only ever pulls from this specific set
    $lastChars = '048AEIMQUYcgkosw'; 
    
    $id = '';
    // Generate first 10 characters normally
    for ($i = 0; $i < 10; $i++) {
        $id .= $chars[random_int(0, 63)];
    }
    
    // Pick the 11th character from the restricted set
    $id .= $lastChars[random_int(0, 15)];
    ...
    <div>
        <div class="outline">
            <div style="display:flex; gap: 10px;">
                <h1>INDIRECT CAST</h1>
                <form name="formbrowser" method="get" action="index.php">
                    <input type="hidden" name="req" value="indirect">
                    <button class="Lucky_Button" name="randomized" value="<?php echo generateAuthenticYoutubeID();?>"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24"><path d="M19 3H5c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2h14c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2zM8 17.5a1.5 1.5 0 1 1 .001-3.001A1.5 1.5 0 0 1 8 17.5zm0-8a1.5 1.5 0 1 1 .001-3.001A1.5 1.5 0 0 1 8 9.5zm4 4a1.5 1.5 0 1 1 .001-3.001A1.5 1.5 0 0 1 12 13.5zm4 4a1.5 1.5 0 1 1 .001-3.001A1.5 1.5 0 0 1 16 17.5zm0-8a1.5 1.5 0 1 1 .001-3.001A1.5 1.5 0 0 1 16 9.5z"></path></svg></button>
                </form>
            </div>
            <div>
        ...
        <div class="file_view" align="center">
            <?php
                if($hold_value) {
                    ...
                    $final_value = $pre_value . $hold_value;
                    if(!empty($hold_value)) {
                        ?>  
        ...
        <div id="modalhistory" onclick="if(event.target === this) closeModalHistory()" style="display:none; position:fixed; z-index:102; left:0; top:0; width:100vw; height:100vh; align-items:center; justify-content:center;">
            <div style="background-color: rgba(0, 0, 0, 0.5);border: 1px solid gray;color: white;padding:15px 15px;border-radius:10px;width:500px;height:500px;text-align:center;position:relative;font-family: 'Roboto', sans-serif; -webkit-backdrop-filter: blur(15px); backdrop-filter: blur(15px); fill: white;">
                ...
                <br>
                <?php
                if (!empty($_SESSION['INDIRECT_History'] ?? [])) {
                    ?>
                    ...
                    <div class="EmptyPlaceholder">
                        ...
                        <p class="BelowPlaceholder">Indirect history empty</p>
                    </div>
                    <?php
                }
                ?>
